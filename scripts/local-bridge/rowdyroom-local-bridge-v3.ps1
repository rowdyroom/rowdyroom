...
$ApprovedRoots = @($ConfigRoot, $DesktopRoot)
if (Test-Path 'D:\') { $ApprovedRoots += 'D:\' }
if (Test-Path 'F:\') { $ApprovedRoots += 'F:\' }
...
function Resolve-ApprovedPath($Path, [bool]$MustExist) {
  ...
  if ($MustExist -and !(Test-Path $full)) { throw "Path does not exist: $full" }
  foreach ($root in $ApprovedRoots) {
    $rootFull = [IO.Path]::GetFullPath($root)
    if (!$rootFull.EndsWith('\')) { $rootFull += '\' }
    if ($full.StartsWith($rootFull, [StringComparison]::OrdinalIgnoreCase) -or $full.Equals($rootFull.TrimEnd('\'), [StringComparison]::OrdinalIgnoreCase)) {
      return $full
    }
  }
  throw "Path is outside approved bridge access: $full"
}
...
function List-Path($Path) {
  $full = Resolve-ApprovedPath $Path $true
  $items = Get-ChildItem -LiteralPath $full -Force | Select-Object Name, FullName, PSIsContainer, Length, LastWriteTime
  return @{ ok = $true; path = $full; items = $items }
}

function Read-TextFile($Path) {
  $full = Resolve-ApprovedPath $Path $true
  $item = Get-Item -LiteralPath $full -Force
  if ($item.PSIsContainer) { throw 'Cannot read a folder as text.' }
  if ($item.Length -gt 2097152) { throw 'File is larger than 2 MB. Use a targeted workflow for large files.' }
  return @{ ok = $true; path = $full; content = Get-Content -LiteralPath $full -Raw -Encoding UTF8 }
}

function Write-TextFile($Path, $Content) {
  $full = Resolve-ApprovedPath $Path $false
  $parent = Split-Path $full -Parent
  Resolve-ApprovedPath $parent $false | Out-Null
...
$listener.Start()
Write-Host "Rowdy Room Local Bridge v3 running at $Prefix" -ForegroundColor Green
Write-Host "Storage folder: $StorageRoot" -ForegroundColor Cyan
Write-Host "Approved roots:" -ForegroundColor Yellow
$ApprovedRoots | ForEach-Object { Write-Host " - $_" -ForegroundColor Yellow }
Write-Host "E drive is not included unless you intentionally change bridge policy later." -ForegroundColor Magenta
...
while ($listener.IsListening) {
  ...
  try {
    ...
    if ($context.Request.HttpMethod -eq 'GET' -and $path -eq 'health') { Write-Json $context 200 @{ ok = $true; bridge = 'rowdyroom-local-bridge-v3'; storageRoot = $StorageRoot; desktopRoot = $DesktopRoot; approvedRoots = $ApprovedRoots; port = $Port; tokenRequired = $true }; continue }
    if ($context.Request.HttpMethod -eq 'GET' -and $path -eq 'roots') { Write-Json $context 200 @{ ok = $true; approvedRoots = $ApprovedRoots; desktopRoot = $DesktopRoot; storageRoot = $StorageRoot }; continue }
    if ($context.Request.HttpMethod -eq 'GET' -and $path -eq 'env/status') { $env = Get-EnvMap; $status = @{}; foreach ($name in $AllowedEnvNames) { $status[$name] = [bool]($env[$name]) }; Write-Json $context 200 @{ ok = $true; env = $status; envPath = $EnvPath; storageRoot = $StorageRoot; approvedRoots = $ApprovedRoots }; continue }

    if ($context.Request.HttpMethod -eq 'POST') {
      $body = Read-BodyJson $context
      if ($path -eq 'env/set') { Backup-Env | Out-Null; Save-EnvValue -Name ([string]$body.name) -Value ([string]$body.value); Write-Json $context 200 @{ ok = $true; updated = [string]$body.name }; continue }
      if ($path -eq 'project/refresh') { $project = Refresh-Code; Write-Json $context 200 @{ ok = $true; project = $project; storageRoot = $StorageRoot }; continue }
      if ($path -eq 'server/stop') { $count = Stop-RowdyNode; Write-Json $context 200 @{ ok = $true; stopped = $count }; continue }
      if ($path -eq 'server/start') { $portNumber = if ($body.port) { [int]$body.port } else { 3000 }; Start-RowdyApp -PortNumber $portNumber; Write-Json $context 200 @{ ok = $true; started = $true; port = $portNumber; storageRoot = $StorageRoot }; continue }
      if ($path -eq 'file/list') { Write-Json $context 200 (List-Path ([string]$body.path)); continue }
      if ($path -eq 'file/read') { Write-Json $context 200 (Read-TextFile ([string]$body.path)); continue }
      if ($path -eq 'file/write') { Write-Json $context 200 (Write-TextFile ([string]$body.path) ([string]$body.content)); continue }
      if ($path -eq 'file/mkdir') { Write-Json $context 200 (Make-Folder ([string]$body.path)); continue }
    }
