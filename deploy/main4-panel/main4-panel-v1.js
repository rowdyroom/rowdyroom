(function installRowdyRoomMain4(global) {
  ...
  const pollMilliseconds = Math.max(2_000, Number(config.pollMilliseconds) || 5_000);
  ...
  async function rpc(name, parameters) {
    ...
    const response = await fetch(`${baseUrl}/rest/v1/rpc/${name}`, {
      ...
      headers: {
        ...
        authorization: `Bearer ${config.accessToken || publishableKey}`,
  ...
  function countdown(endValue) {
    ...
    const seconds = Math.max(0, Math.ceil((new Date(endValue).getTime() - Date.now()) / 1_000));
  ...
  function render(root, panel) {
    ...
    mode.textContent = panel.mode === "rumble_protected" ? countdown(panel.protection_ends_at) : panel.current_rumble_session_id ? "Rumble in progress" : "Live score + gift formula";
